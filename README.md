...
* **Trouble Shooting**:
    ...
    * **배운점**: 서로 다른 형식의 공공 데이터를 병합하고 StandardScaler로 정규화하는 과정을 통해, 실무 데이터의 결측치 및 스케일 불일치 문제를 해결하는 데이터 전처리 파이프라인 구축 능력을 길렀습니다.
비즈니스 인사이트 도출: 단순한 모델링을 넘어, 비즈니스 목적에 맞는 독자적인 지표(Feature)를 설계하고 가중치를 부여하는 분석 기획 역량이 AI 모델의 실질적인 활용도를 결정짓는 핵심임을 깨달았습니다.
